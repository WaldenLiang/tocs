<template>
  <section class="o-block-card"
           :class="{ 'fullscreen': fullScreen, 'selected': selected }">
    <section class="row col-12 o-card-toolbar"
             v-if="toolbar">
      <div class="row col">
        <div class="q-px-sm"
             v-if="fullScreen">
      ...
      <div class="row col-auto q-pl-sm">
        <q-btn icon="fullscreen"
               dense
               flat
               @click="onToggleScreen">
          <q-tooltip anchor="top middle"
                     self="bottom middle">{{$t('editor.toggleFullscreen')}}
          </q-tooltip>
        </q-btn>
        <slot name="toolbar-right"></slot>
      </div>
    </section>
    <section class="o-card-inner-toolbar"
             v-else>
      <q-btn icon="fullscreen"
             color="blue"
             round
